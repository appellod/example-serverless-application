...
import { BaseController } from "../../../../src/microservices/rest/controllers";
import { User, UserPermissions } from "../../../../src/common/postgres";
import { UserMock } from "../../../common/postgres/mocks";

class TestController extends BaseController<User, UserPermissions> {
  constructor() {
  ...
  describe("find()", function() {
    ...
    beforeEach(async function() {
      users = await Promise.all([
        UserMock.insert({ id: 1 }),
        UserMock.insert({ id: 2 }),
        UserMock.insert({ id: 3 })
      ]);
    });
    ...
    it("enforces where permissions", async function() {
      const admin = await UserMock.insert({ level: 1 });
      const params = {
